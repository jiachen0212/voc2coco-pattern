...
class MSCOCO(DETECTION):
    def __init__(self, db_config, split):
        ...
        self._split = split
        print ('cur split: ', self._split)
        self._dataset = {
        "trainval": "voc2012trainval",
        "minival": "voc2007val",
        "testdev": "voc2012test"}[self._split]

        self._coco_dir = os.path.join(data_dir, "voc_coco_pattern")
        ...
        self._cache_file = os.path.join(cache_dir, "voc_coco_{}.pkl".format(self._dataset))
        self._load_data()
    ...
    def _load_coco_data(self):
        print ('the anno file: ', self._label_file)
    ...
    def _extract_data(self):
        self._coco    = COCO(self._label_file)
        print ('annotation json: ', self._label_file)
        ...
        print ('training datasize: ', len(self._image_ids))
        self._detections = {}
        for ind, (coco_image_id, image_id) in enumerate(tqdm(zip(coco_image_ids, self._image_ids))):
            ...
            for cat_id in self._cat_ids:
                ...
                # print ('ann_id:', annotation_ids)
                # annotations    = self._coco.loadAnns(annotation_ids)
                # self._coco_to_class_map.get(key)   # check the dict
                ...
                if len(annotation_ids) > 0:
                    # print('ann_id:', annotation_ids)
                    annotations = self._coco.loadAnns(annotation_ids)
                    # print ('annoations:', annotations)
                    # print ('cur cate: ', cat_id)
                    # self._coco_to_class_map.get(key)  # check the dict
                    category = self._coco_to_class_map[cat_id]
